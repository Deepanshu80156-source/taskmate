...
**Status:** Web/PWA build-verified; Android wrapper synchronized
...
- `supabase/20260821_taskmate_repairs.sql` is the targeted migration for the live repair: student creation, password reset, avatar visibility, and teacher deletion permissions.
...
1. Extract this folder and place `.env.local` directly beside this `package.json` (copy `.env.example` to `.env.local`; never put it in `src/` or `android/`).
2. Run `pnpm install` or `npm install`, then `npm run typecheck` and `npm run build`.
3. In the existing Supabase project, run the canonical schema once, then run `supabase/20260821_taskmate_repairs.sql`.
4. Test teacher signup, student registration/login, password reset, profile photos, notes, results, announcements, library, messages, deletion actions, and notifications before pushing.
5. For Android, install Android Studio/JDK, set `ANDROID_HOME`, run `npm run build`, `npx cap sync android`, then `cd android && ./gradlew assembleDebug`. The debug APK is written under `android/app/build/outputs/apk/debug/`.
...
- 📦 **Android APK** – Capacitor Android project included for debug and signed release builds
...
This package includes an npm lockfile and explicit dependency versions, so a
fresh checkout can be installed reproducibly:
...
The repository includes the complete Capacitor Android wrapper. A release APK must be
signed with your own keystore; keystores and signing credentials are intentionally not
included in this ZIP. Use Android Studio or the Gradle command above after installing
the Android SDK and accepting its licenses.
