...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const removeResult = async (id: string) => {
    if (!currentUser || currentUser.role !== 'teacher') {
      throw new Error('Only teachers can delete results.');
    }
    const target = results.find((item) => item.id === id && item.teacherId === currentUser.id);
    if (!target) throw new Error('This result is not assigned to you.');
    const { error } = await supabase.from('results').delete().eq('id', id).eq('teacher_id', currentUser.id);
    if (error) throw new Error(describeSupabaseError(error, 'The result could not be deleted'));
  ...
  const removeMessage = async (messageId: string) => {
    if (!currentUser || currentUser.role !== 'teacher') {
      throw new Error('Only teachers can delete messages.');
    }
    const conversation = conversations.find((item) =>
      item.messages.some((message) => message.id === messageId),
    );
    if (!conversation) throw new Error('Message not found.');
    ...
    const { error } = await supabase
      ...
      .delete()
      .eq('id', messageId);
    if (error) throw new Error(describeSupabaseError(error, 'The message could not be deleted'));

    if (message.attachmentPath) {
      await supabase.storage.from('message_files').remove([message.attachmentPath]);
    }
    setConversations((previous) =>
      previous.map((item) =>
